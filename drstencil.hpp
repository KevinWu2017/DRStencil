#ifndef __DRSTENCIL_HPP__
#define __DRSTENCIL_HPP__
...
class DRStencil
{
	private:
		...
		std::map<std::tuple<int, int, int>, float> stencil, fused;
		void do_fusing (int, int, int, float, int);
	public:
...
void DRStencil::do_fusing(int k, int j, int i, float coe, int step)
{
	if(step == 0) {
