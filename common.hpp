...
static float* getRandom2DArray(int width_y, int width_x) {
  float (*a)[width_x] = (float (*)[width_x])new float[width_y*width_x];
...
static float* getZero2DArray(int width_y, int width_x) {
  float (*a)[width_x] = (float (*)[width_x])new float[width_y*width_x];
  memset((void*)a, 0, sizeof(float) * width_y * width_x);
  return (float*)a;
}
...
static float checkError2D
(int width_x, const float *l_output, const float *l_reference, int y_lb, int y_ub,
 int x_lb, int x_ub) {
  const float (*output)[width_x] = (const float (*)[width_x])(l_output);
  const float (*reference)[width_x] = (const float (*)[width_x])(l_reference);
  float error = 0.0;
  ...
  for (int j = y_lb; j < y_ub; j++) 
    for (int k = x_lb; k < x_ub; k++) {
      ...
      float curr_error = output[j][k] - reference[j][k];
